...
from src.services.artifact_infer.utils.context import set_cuda_graph_flag

from src.artifacts.artifact_infer.cache_mngr.layerwise import CacheManager

from src.artifacts.artifact_infer.cache_mngr.snapKV import SnapKV
from src.artifacts.artifact_infer.cache_mngr.snapKV_new import SnapKVNew
from src.artifacts.artifact_infer.cache_mngr.RKV import RKV
from src.artifacts.artifact_infer.cache_mngr.RKV_new import RKVNew
from src.artifacts.artifact_infer.cache_mngr.sliding import SlidingWindowKV
from src.artifacts.artifact_infer.cache_mngr.sliding_new import SlidingWindowKVNew
from src.artifacts.artifact_infer.cache_mngr.query import QueryAwareKV
from src.artifacts.artifact_infer.cache_mngr.question_new import QuestionAwareKVNew
from src.artifacts.artifact_infer.cache_mngr.tabula import TabulaKV

from src.services.artifact_infer.model_runner.models.qwen3 import Qwen3AttentionArtifacts
...
class ModelRunner(BaseService):
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        if self.world_size > 1:
            if rank == 0:
                self.shm = SharedMemory(name="artifact_infer", create=True, size=2**20)
                dist.barrier()
            else:
                dist.barrier()
                self.shm = SharedMemory(name="artifact_infer")
                self.loop()
