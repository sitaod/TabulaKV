...
class QueryAwareKV:
    """One-shot question-aware KV compression.

    The prompt question tokens score historical KV tokens. Compression keeps
    the most relevant history tokens and always keeps the most recent window.
    """
    ...
    def update_kv(self, query_states, key_states, value_states):
        ...
        num_history_tokens = self.budget - self.window_size
        indices = history_scores.topk(num_history_tokens, dim=-1).indices
        ...
        return torch.cat([k_history, k_recent], dim=2), torch.cat(
            [v_history, v_recent],
            dim=2,
        )
